 ...
 * Called by {@link MyContextLoaderListener}.
 *
 ...
 * if not found. With the default MyContextLoader implementation, any context class
 ...
 *
 * <p>As of Spring 3.1, {@code MyContextLoader} supports injecting the root web
 * application context via the {@link #MyContextLoader(WebApplicationContext)}
...
public class MyContextLoader {
    ...
    private static final Logger log = LoggerFactory.getLogger(MyContextLoader.class);
    ...
    /**
     * Name of the class path resource (relative to the MyContextLoader class)
     * that defines MyContextLoader's default strategy names.
     */
    private static final String DEFAULT_STRATEGIES_PATH = "MyContextLoader.properties";
    ...
    static {
        ...
        try {
            ClassPathResource resource = new ClassPathResource(DEFAULT_STRATEGIES_PATH, MyContextLoader.class);
            defaultStrategies = PropertiesLoaderUtils.loadProperties(resource);
        } catch (IOException ex) {
            throw new IllegalStateException("Could not load 'MyContextLoader.properties': " + ex.getMessage());
        }
    }
    ...
    /**
     * The 'current' WebApplicationContext, if the MyContextLoader class is
    ...
    /**
     * Create a new {@code MyContextLoader} that will create a web application context
     ...
     * MyContextLoaderListener} subclass as a {@code <listener>} within {@code web.xml}, as
     ...
     *
     * @see #MyContextLoader(WebApplicationContext)
    ...
    /**
     * Create a new {@code MyContextLoader} with the given application context. This
     ...
     * @see #closeWebApplicationContext(ServletContext)
     */
    public MyContextLoader(WebApplicationContext context) {
     ...
     * to initialize the application context used by this {@code MyContextLoader}.
     *
    ...
    public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
        log.info("test;this.context={};", this.context);
        if (servletContext.getAttribute(WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE) != null) {
            throw new IllegalStateException(
                    "Cannot initialize context because there is already a root application context present - " +
                            "check whether you have multiple MyContextLoader* definitions in your web.xml!");
        }

        Log logger = LogFactory.getLog(MyContextLoader.class);
        ...
        try {
            ...
            if (this.context instanceof ConfigurableWebApplicationContext) {
                ...
                boolean active = cwac.isActive();
                log.info("test;active={};", active);
                if (!active) {
                    // The context has not yet been refreshed -> provide services such as
                    // setting the parent context, setting the application context id, etc
                    ApplicationContext cwacParent = cwac.getParent();
                    log.info("test;cwacParent={};", cwacParent);
                    if (cwacParent == null) {
            ...
            if (ccl == MyContextLoader.class.getClassLoader()) {
    ...
    protected Class<?> determineContextClass(ServletContext servletContext) {
        ...
        if (contextClassName != null) {
            ...
            try {
                return ClassUtils.forName(contextClassName, MyContextLoader.class.getClassLoader());
            } catch (ClassNotFoundException ex) {
                throw new ApplicationContextException(
     ...
     * MyContextLoader after config locations have been supplied to the context
    ...
    protected ApplicationContext loadParentContext(ServletContext servletContext) {
        ...
        log.info("test;parentContextKey={};", parentContextKey);
        if (parentContextKey != null) {
            ...
            log.info("test;locator={};this.parentContextRef={};",locator,this.parentContextRef);
            parentContext = (ApplicationContext) this.parentContextRef.getFactory();
        }
        log.info("test;parentContext={};", parentContext);
