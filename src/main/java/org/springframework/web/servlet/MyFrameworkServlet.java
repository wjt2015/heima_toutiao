 ...
 * {@code MyFrameworkServlet}, a custom context class needs to implement the
 ...
 *
 * <p>As of Spring 3.1, {@code MyFrameworkServlet} may now be injected with a web
 * application context, rather than creating its own internally. This is useful in Servlet
 * 3.0+ environments, which support programmatic registration of servlet instances. See
 * {@link #MyFrameworkServlet(WebApplicationContext)} Javadoc for details.
 *
 * @author Rod Johnson
...
public abstract class MyFrameworkServlet extends MyHttpServletBean implements ApplicationContextAware {
    ...
    /**
     * Default context class for MyFrameworkServlet.
     ...
     */
    public static final String SERVLET_CONTEXT_PREFIX = MyFrameworkServlet.class.getName() + ".CONTEXT.";
    ...
    /**
     * Create a new {@code MyFrameworkServlet} that will create its own internal web
     ...
     * @see #MyFrameworkServlet(WebApplicationContext)
     */
    public MyFrameworkServlet() {
    }

    /**
     * Create a new {@code MyFrameworkServlet} with the given web application context. This
     ...
     */
    public MyFrameworkServlet(WebApplicationContext webApplicationContext) {
     ...
     * <p>When using the default MyFrameworkServlet implementation,
     ...
     * to initialize the application context used by this {@code MyFrameworkServlet}.
     * @see #configureAndRefreshWebApplicationContext
     ...
     * <p>Default in the {@code MyFrameworkServlet} is "false", applying
    ...
    protected final void initServletBean() throws ServletException {
        getServletContext().log("Initializing Spring MyFrameworkServlet '" + getServletName() + "'");
        if (this.logger.isInfoEnabled()) {
            this.logger.info("MyFrameworkServlet '" + getServletName() + "': initialization started");
        }
        ...
        if (this.logger.isInfoEnabled()) {
            ...
            this.logger.info("MyFrameworkServlet '" + getServletName() + "': initialization completed in " +
                    elapsedTime + " ms");
        }
    }
    ...
    @Override
    public void destroy() {
        getServletContext().log("Destroying Spring MyFrameworkServlet '" + getServletName() + "'");
     ...
     * only, delegating to {@code onApplicationEvent} on the MyFrameworkServlet instance.
     */
     ...
     * MyFrameworkServlet's context holders, i.e. LocaleContextHolder and RequestContextHolder.
     */
